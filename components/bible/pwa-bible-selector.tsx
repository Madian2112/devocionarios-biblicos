...
import { useBibleModal } from "@/hooks/use-bible-modal";
...
export function BibleSelector({
  ...
  const { isOpen, modalData, openModal, closeModal } = useBibleModal(instanceId);
  ...
  const FormContent = (
    <BibleSelectorForm
      onSelect={modalData?.onSelect || onSelect}
      onClose={handleCloseModal}
      currentReference={modalData?.currentReference || currentReference}
  ...
  return (
    <>
      ...
      {isOpen && (
        <>
          {isMobile ? (
            <Drawer open={true} onOpenChange={(open) => !open && handleCloseModal()}>
              <DrawerContent className="bg-[#1a1a1a] border-gray-800 text-white">
                <DrawerHeader className="text-left">
                  <DrawerTitle>Seleccionar Pasaje Bíblico</DrawerTitle>
                  <DrawerDescription>Elige el libro, capítulo y versículo(s).</DrawerDescription>
          ...
          <SelectContent className="bg-[#1a1a1a] border-gray-800 text-white max-h-60">
            <ScrollArea className="h-60">
              {loadingBooks ? (
                ...
                books.map((book) => (
                  <SelectItem
                    key={`book-${LibroApocalpsis(book.name)}`}
              ...
              <SelectContent className="bg-[#1a1a1a] border-gray-800 text-white max-h-60">
                <ScrollArea className="h-60">
                  {Array.from({ length: maxChapters }, (_, i) => i + 1).map(c => 
                    <SelectItem key={c} value={c.toString()} className="hover:bg-[#2a2a2a]">{c}</SelectItem>
                  )}
                </ScrollArea>
              </SelectContent>
            </Select>
              ...
              <SelectContent className="bg-[#1a1a1a] border-gray-800 text-white max-h-60">
                <ScrollArea className="h-60">
                  {Array.from({ length: maxVerses }, (_, i) => i + 1).map(v => 
                    <SelectItem key={v} value={v.toString()} className="hover:bg-[#2a2a2a]">{v}</SelectItem>
                  )}
                </ScrollArea>
              </SelectContent>
            </Select>
