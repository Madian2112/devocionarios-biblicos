// 3. BIBLE VIEWER CORREGIDO
// components/bible/BibleViewer.tsx
...
interface BibleViewerProps {
  instanceId: string; // Ahora requerido
  reference: string;
...
export function BibleViewer({
  ...
  const handleCloseModal = () => {
    if (modalData?.onClose) {
      modalData.onClose(selectedVersion);
    }
    closeModal();
  };
  ...
  const ViewerContent = (
    <BibleViewerContent
      ...
      reference={modalData?.reference || reference}
      isOpen={isOpen}
      defaultVersion={modalData?.defaultVersion || defaultVersion}
...
export function BibleViewerContent({
  ...
  }, [isOpen, selectedVersion, reference]);
