...
import { BibleSelector } from "@/components/bible/pwa-bible-selector"
import { BibleViewer } from "@/components/bible/pwa-bible-viewer"
...
import { usePWACleanup } from "@/hooks/use-pwa-cleanup"
...
const handleVersiculoChange = (index: number, updates: Partial<Versiculo>) => {
  ...
  if (index < 0 || index >= updatedVersiculos.length || !updatedVersiculos[index]) {
    ...
    console.error(
      ...
      updatedVersiculos.map((v, i) => ({ index: i, id: v.id, referencia: v.referencia }))
    );
    return;
  }

  // Actualizar ambos campos en un solo paso
  updatedVersiculos[index] = { ...updatedVersiculos[index], ...updates };

  console.log('Estos son mis versiculos:', updatedVersiculos);
        ...
        <CardContent className="space-y-6">
          <div>
            ...
            <div className="flex gap-3 items-center">
              ...
              {/* BIBLE SELECTOR PARA VERSÍCULO - INSTANCIA ÚNICA */}
              <BibleSelector
              ...
              {/* BIBLE VIEWER PARA VERSÍCULO - INSTANCIA ÚNICA */}
